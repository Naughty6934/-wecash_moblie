angular.module('your_app_name.app.services', [])
  ...
  .service('AuthService', function ($http, $q) {
    ...
    this.signUp = function (user) {
      ...
      $http.post('http://localhost:3000/api/auth/signup', user).success(function (database) {
    ...
    this.login = function (user) {
      ...
      $http.post('http://localhost:3000/api/auth/signin', user).success(function (database) {
        window.localStorage.user = JSON.stringify(database);
        dfd.resolve(database);
      }).error(function (error) {
        dfd.reject(error)
  ...
  //////Applications
  .service('ExchangeService', function ($http, $q) {

    this.getExchangerate = function (base) {
      var dfd = $q.defer();

      $http.get('http://localhost:3000/api/exchangesrate/' + base).success(function (database) {
